...
var join = {
    reg: function() {
       ...
       if (login.length == 0 || login.length < 6 || pass.length == 0 || pass.length < 6) {
           ...
           error.innerHTML = lang_error;
        } else {
           error.style.display = "none";
           status.style.color = "#5FB053";
           status.innerHTML = lang_processing;
           ajax.post({
               url: "/",
               ...
               callback: function(data) {
                    ...
                    if (data == "Err1") {
                        error.style.display = "block";
                        error.innerHTML = lang_error_reg;
                        status.innerHTML = "";
                    } else if (data == "Err2") {
                        error.style.display = "block";
                        error.innerHTML = lang_sign_email;
                        status.innerHTML = "";
                    } else 
            ...
            ajax.post({
                ...
                callback: function(data) {
                    ...
                    if (data == "Err1") {
                        error.style.display = "block";
                        status.innerHTML = "";
                        error.innerHTML = lang_login_error;
                    } else if (data == "Err2") {
...
var lang_ = {
    openBox: function() {
        var ajax_block = ge("ajax_load_block_");
        var div_content = document.createElement("div");
        div_content.classList.add("lang-div-content");
    
        var div_header = document.createElement("div");
        div_header.classList.add("lang-div-header");
        div_header.innerHTML = lang_box_header_l;
        div_content.appendChild(div_header);
        
        var div_c = document.createElement("div");
        div_c.classList.add("lang-div-c");
        div_c.setAttribute("onclick", "lang_.selectLang(event)");
        div_c.innerHTML = "<span class=\"lang-d\" data-lang=\"ru\">Русский</span><span class=\"lang-d\" data-lang=\"en\">English</span>";
        div_content.appendChild(div_c);
        
        var div_f = document.createElement("div");
        div_f.classList.add("lang-div-f");
        div_f.innerHTML = "<button class=\"main-button btn-f\" onclick=\"lang_.closeBox()\">Закрыть</button>";
        
        div_content.appendChild(div_f);
    
        ajax_block.appendChild(div_content);
        ajax_block.style.display = "block";
    },
    closeBox: function() {
        var ajax_block = ge("ajax_load_block_");
    ...
    processing: function() {
        ...
        ajax.a(url, {
           callback: function(data) {
               ...
               //var previewImageSize = ge("preview_image_size");
               //previewImageSize.src = data;
...
var feedback = {
    showBox: function(display) {
        ...
        var report_block = document.createElement("div");
        report_block.className = "report-block";
        
        var header_report_block = document.createElement("div");
        header_report_block.className = "report-block-header";
        header_report_block.innerHTML = "Сообщить об ошибке";
        
        var content_report_block = document.createElement("div");
    ...
    send: function() {
        var feedback_error_descr = ge("feedback_error_descr").value;
        ...
        var url = "/?act=feedback&message=" + feedback_error_descr;
        loading("start");
        ajax.a(url, {
            callback: function(data) {
                loading("stop");
...
var report = {
    ...
    display: function(display, id) {
        ...
        else header_report_block.innerHTML = "Жалоба на пользователя";
        
        var content_report_block = document.createElement("div");
        content_report_block.className = "report-block-content";
        
        if (this.type == "photo") { 
          content_report_block.innerHTML = "Пожалуйста, выберите причину, по которой Вы хотите сообщить администрации сайта о нарушении: <div class=\"report-block-form\"><select class=\"report-select\" id=\"report_theme\"><option value=\"0\">Выбирите причину</option><option value=\"1\">Порнография</option><option value=\"2\">Оскорбительное содержание</option><option value=\"3\">Спам</option><option value=\"4\">Другая причина</option></select><b style=\"display: block; padding: 20px 0 0\">Комментарий:</b><textarea id=\"descr_report\" class=\"report-textarea\" placeholder=\"Ваш комментарий: \"></textarea></div>Если данная фотография не соответсвует выбранной категории, то укажите \"Другая причина\" и <b>обязятально</b> напишите об этом в комментирий жалобы.<div class=\"report-footer\" style=\"padding-top: 30px;\"><span id=\"report_status\"></span><button class=\"main-button\" onclick=\"report.send(" + id + ")\">Отправить</button><button class=\"main-button btn-f\" onclick=\"report.hide()\">Закрыть</button></div>";
        } else {
          content_report_block.innerHTML = "Пожалуйста, сообщите нам причину, по которой страница должна быть заблокирована: <div class=\"report-block-form\"><select class=\"report-select\" id=\"report_theme\"><option value=\"0\">Выбирите причину</option><option value=\"1\">Порнография</option><option value=\"2\">Оскорбительное содержание</option><option value=\"3\">Спам</option><option value=\"4\">Другая причина</option></select><b style=\"display: block; padding: 20px 0 0\">Комментарий:</b><textarea id=\"descr_report\" class=\"report-textarea\" placeholder=\"Опишите проблему: \"></textarea></div>Если Вам данный пользователь наносит какой-то моральный ущерб, то Вы можете добавить его в черный список.<div class=\"report-footer\" style=\"padding-top: 30px;\"><span id=\"report_status\"></span><button class=\"main-button\" onclick=\"report.send(" + id + ")\">Отправить</button><button class=\"main-button btn-f\" onclick=\"report.hide()\">Закрыть</button></div>";
        }
    ...
    send: function(id) {
        var report_theme = ge("report_theme").value;
        var report_comment = ge("descr_report").value;
        ...
        var url = "/?act=report&type=" + this.type + "&rid=" + id + "&theme=" + report_theme + "&comment=" + report_comment;
        loading("start");
        ajax.a(url, {
            callback: function(data) {
                loading("stop");
...
var comment = {
    s: function(id) {      
        var url = "/?act=comment&post_id=" + id;
...
function createCss(t) {
	 ...
	 css.rel = "stylesheet";
	 css.media = "screen";
	 css.href = "/tpl/css/" + t + ".css?12124.0";
...
var promotion = {};
promotion.showDialogPromotion = function(pid) {
    var url = "/?act=promotion&p=showDialog&post_id=" + pid;
    loading("start");
    ajax.a(url, {
        callback: function(data) {
            loading("stop");
            var ajax_page = ge("ajax_load_block");
            ajax_page.innerHTML = data;
        }
    });
}
promotion.defaultCntDays = "days1";
promotion.selectCntDays = function(id) {
    if (id != promotion.defaultCntDays) {
        ge(promotion.defaultCntDays).classList.remove("promotion-btn-active");
        promotion.defaultCntDays = id;
        ge(id).classList.add("promotion-btn-active");
    } 
}
promotion.promotionPublic = function(pid) {
    var url = "/?act=promotion&p=publication&post_id=" + pid + "&count_days=" + promotion.defaultCntDays;
    var status_p = ge("status_p");
    var warning_info = ge("warn_info_p");
    var warning_text = ge("warn_info_p_text");
    status_p.style.display = "block";
    ajax.a(url, {
        callback: function(data) {
            status_p.style.display = "none";
            if (data == 0) {
                confirmBox.info("Продвижение публикации проведено успешно. Скоро она появится в Вашем профиле и рекомендациях Ваших подписчиков.");
            } else if (data == 1) {
                warning_info.style.display = "block";
                warning_text.innerHTML = "К сожалению, у Вас недостаточно репутации, чтобы оплатить продвижение публикации. Вы можете заработать репутации за фотографии, а также приобрести ее в нашем магазине.";
            } else {
                warning_info.style.display = "block";
                warning_text.innerHTML = "К сожалению, произошла ошибка. Повторите попытку позже.";
            }
        }
    });
}
...
var restore = {
    submitForm: function() {
        var r_input = ge("r_email");
        if ((r_input.value).length == 0 || (r_input.value).length < 8) {
            r_input.focus();
            return false;
        }
        document.location.href = "/restore?act=r&email=" + r_input.value;
    },
    noAccount: function() {
        document.location.href = "/restore";
    },
    finish: function() {
        document.location.href = "/restore?act=finish";
    }
}
...
window.onscroll = function() {
  ...
  if (scrollTop + windowHeight >= (documentHeight - 300) * (cnt - 1) && !flag) {
    var url = "/?act=ajax&href=" + document.location.pathname + "&from=" + cnt;
    ajax.a(url, {
      callback: function(data) {
        if (data == 1) {
            flag = true;
        ...
        switch (document.location.pathname) {
            case "/": case "/lenta": main_container = ge("lenta_main"); break;
            case "/photo": main_container = ge("album_main"); break;
        }
        var main = main_container.innerHTML;
        main += data;
        main_container.innerHTML = main;
      }
    });
    cnt += 10;
  }
...
/*
var url = "/scripts/NotifyServer.php";
var timeNotify = 0;
setInterval(function() {
	var newNotifySpan = ge("new_notify");
           var newNotifySpanMobile = ge("new_notify_mobile");

           if (newNotifySpan == null || newNotifySpanMobile == null) return;
   ajax.a(url, {
       callback: function(data) {
           var newNotifySpan = ge("new_notify");
           var newNotifySpanMobile = ge("new_notify_mobile");
           ...
           if (data == 1) {
             newNotifySpan.classList.remove("none");
             newNotifySpanMobile.classList.remove("none");
           } else {
             newNotifySpan.classList.add("none");
             newNotifySpanMobile.classList.add("none");
           }
           timeNotify = 10000;
       }
   }); 
